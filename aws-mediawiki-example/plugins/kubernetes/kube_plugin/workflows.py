...
#
# Scale resource to a given number of replicas
#
@workflow
def kube_scale(**kwargs):
  setfabenv(kwargs)
  optstr=buildopts(kwargs,{},{},["all"],['name','master','resource'])
  runstr="./kubectl -s http://localhost:8080 scale {} {} {}".format(kwargs['resource'],kwargs['name'],optstr)
  ctx.logger.info("Running: {}".format(runstr))
  run(runstr)
...
def get_ip(master):
  if(ctx.local):
    ...
    raise Exception('not implemented')  # need to get default instance in cloud case
